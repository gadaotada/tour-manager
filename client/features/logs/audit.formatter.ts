...
type AuditFormatResource = Extract<AuditResource, "HOTELS" | "USERS">;
...
function formatGeneralAuditLog(audit: AuditLog, t: Translate): string {
  const actor = audit.actor_display_name || audit.actor_username || t("logs.formatter.actorFallback");
  ...
  switch (audit.action) {
    case "CREATE":
      return joinAuditParts([
        actor,
        t("logs.formatter.actions.CREATE"),
        resource,
      ]);
    case "UPDATE":
      return joinAuditParts([
...
function formatCreateAuditValues(data: AuditLog["data"], t: Translate): string {
  ...
  return entries
    .map(([field, value]) => `${formatFieldLabel(field, t)}: ${formatAuditValue(field, value, t)}`)
    .join(", ");
}
...
function formatBooleanAuditValue(field: string, value: boolean, t: Translate): string {
  if (field === "is_active") {
    return value
      ? t("logs.formatter.values.active")
      : t("logs.formatter.values.inactive");
  }

  if (field === "is_enabled") {
    return value
      ? t("logs.formatter.values.enabled")
      : t("logs.formatter.values.disabled");
  }
...
function formatAuditArrayValue(value: unknown[], t: Translate): string {
  ...
  return t(value.length === 1 ? "logs.formatter.values.oneItem" : "logs.formatter.values.manyItems")
...
function formatAuditObjectValue(value: object, t: Translate): string {
  try {
    ...
    return t("logs.formatter.changedFields.changed").replace("{field}", t("logs.formatter.fields.value"));
  }
}

function isKnownFormatResource(resource: AuditResource): resource is AuditFormatResource {
  return resource === "HOTELS" || resource === "USERS";
}
...
function isPermissionOverrideValue(value: unknown): value is { effect: string; permission: string } {
  return (
    isPlainRecord(value) &&
